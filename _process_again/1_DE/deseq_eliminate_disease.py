...
def main():
    ...
    # Contrast on the "condition" factor: log2FC(Disease vs Control)
    stats = DeseqStats(
        dds,
        contrast=["gender", "Female", "Male"],
        inference=inf
    )
    stats.summary()
    res = stats.results_df
    res.to_csv('/l/users/darya.taratynova/scbb_project/_process_again/1_DE/results/disease.csv')
